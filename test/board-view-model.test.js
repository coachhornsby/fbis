import { describe, it } from "node:test";
import assert from "node:assert/strict";
import {
  buildBoardGameViewModel,
  buildDecisionPresentation,
  resolveBoardMarketLabel,
} from "../src/lib/boardViewModel.js";
import {
  DEFAULT_OPERATOR_EXECUTION_BOOKS,
  resolveCanonicalMarket,
} from "../functions/lib/canonical/marketRoles.js";

function researchNflGame(overrides = {}) {
  return {
    id: "nfl-1",
    sport: "nfl",
    start: "2026-09-14T00:20:00Z",
    away: { abbr: "DAL", name: "Cowboys" },
    home: { abbr: "NYG", name: "Giants" },
    model: {
      projAway: 30.3,
      projHome: 30.7,
      projTotal: 61,
      projMargin: 0.4,
    },
    projectionMaturity: "RESEARCH",
    researchProjection: true,
    publicationStatus: "RESEARCH_PUBLISHABLE",
    pureProjectionAvailable: true,
    projectionKind: "FBIS",
    market: {
      marketAvailable: true,
      execution: {
        available: true,
        book: "FanDuel",
        spread: 3,
        total: 47.5,
        moneyline: { home: -110, away: -110 },
      },
      consensus: { available: false },
      reference: { available: false },
    },
    ...overrides,
  };
}

describe("Board game view-model", () => {
  it("surfaces FBIS projection prominently for research models", () => {
    const vm = buildBoardGameViewModel(researchNflGame());
    assert.equal(vm.projection.available, true);
    assert.equal(vm.projection.research, true);
    assert.match(String(vm.projection.headlineLabel), /FBIS/);
    assert.equal(vm.projection.away, 30.3);
    assert.equal(vm.projection.home, 30.7);
  });

  it("one execution offer → EXECUTION OFFER, not BEST AVAILABLE", () => {
    const vm = buildBoardGameViewModel(researchNflGame());
    assert.equal(vm.market.available, true);
    assert.equal(vm.market.referenceOnly, false);
    assert.equal(vm.market.book, "FanDuel");
    assert.equal(vm.market.label, "EXECUTION OFFER");
    assert.notEqual(vm.market.label, "BEST AVAILABLE");
  });

  it("legitimately ranked multiple execution offers → BEST AVAILABLE", () => {
    const vm = buildBoardGameViewModel(
      researchNflGame({
        market: {
          marketAvailable: true,
          execution: {
            available: true,
            book: "FanDuel",
            spread: 3,
            total: 47.5,
            moneyline: { home: -110, away: -110 },
            likeForLike: true,
            rankedBest: true,
            comparableOfferCount: 2,
            alternativesInLine: [{ book: "DraftKings", price: -105 }],
            rankLabel: "BEST_LIKE_FOR_LIKE",
          },
          consensus: { available: false },
          reference: { available: false },
        },
      })
    );
    assert.equal(vm.market.label, "BEST AVAILABLE");
  });

  it("consensus-only → CONSENSUS", () => {
    const vm = buildBoardGameViewModel(
      researchNflGame({
        market: {
          marketAvailable: true,
          execution: { available: false },
          consensus: {
            available: true,
            source: "ACTION",
            spread: 3,
            total: 47,
          },
          reference: { available: false },
        },
      })
    );
    assert.equal(vm.market.available, true);
    assert.equal(vm.market.label, "CONSENSUS");
  });

  it("non-executable observed feed → OBSERVED MARKET", () => {
    const vm = buildBoardGameViewModel(
      researchNflGame({
        market: {
          marketAvailable: true,
          execution: { available: false },
          consensus: {
            available: true,
            source: "OBSERVED",
            observedOnly: true,
            spread: 3,
            total: 47,
          },
          reference: { available: false },
        },
      })
    );
    assert.equal(vm.market.label, "OBSERVED MARKET");
  });

  it("reference-only → REFERENCE ONLY", () => {
    const vm = buildBoardGameViewModel(
      researchNflGame({
        market: {
          marketAvailable: false,
          execution: { available: false },
          consensus: { available: false },
          reference: {
            available: true,
            provider: "Pinnacle",
            spread: 2.5,
            total: 47,
            moneyline: { home: -105, away: -115 },
          },
        },
      })
    );
    assert.equal(vm.projection.available, true);
    assert.equal(vm.market.available, false);
    assert.equal(vm.market.referenceOnly, true);
    assert.equal(vm.market.label, "REFERENCE ONLY");
    assert.ok(vm.market.emptyReason);
  });

  it("no market anywhere → NO CURRENT MARKET", () => {
    const vm = buildBoardGameViewModel(
      researchNflGame({
        market: {
          marketAvailable: false,
          execution: { available: false },
          consensus: { available: false },
          reference: { available: false },
        },
      })
    );
    assert.equal(vm.market.available, false);
    assert.equal(vm.market.label, "NO CURRENT MARKET");
    assert.ok(vm.market.emptyReason);
  });

  it("labels disagreement as model difference — not EV — for research", () => {
    const vm = buildBoardGameViewModel(researchNflGame());
    assert.equal(vm.authority.research, true);
    assert.equal(vm.authority.evAvailable, false);
    assert.equal(vm.decision.ev, null);
    assert.equal(vm.decision.qualification, "RESEARCH_ONLY");
    assert.match(String(vm.comparison.label), /DIFFERENCE|MODEL/i);
    assert.equal(vm.comparison.hasDiff, true);
  });

  it("does not invent qualification or authorization for research", () => {
    const d = buildDecisionPresentation(researchNflGame());
    assert.equal(d.qualification, "RESEARCH_ONLY");
    assert.notEqual(d.qualification, "QUALIFIED");
    assert.equal(d.evAvailable, false);
    assert.equal(d.humanConfirmationRequired, true);
    assert.ok(d.authorization === "NONE" || d.authorization === "AWAITING_REVIEW");
  });

  it("stale execution cannot produce an actionable decision", () => {
    const d = buildDecisionPresentation(
      researchNflGame({
        market: {
          marketAvailable: true,
          executionMarketAvailable: true,
          executionActionable: false,
          execution: {
            available: true,
            book: "FanDuel",
            spread: 3,
            total: 47.5,
            moneyline: { home: -110, away: -110 },
            actionable: false,
            freshness: "STALE",
          },
          consensus: { available: false },
          reference: { available: false },
        },
        decision: { tier: "QUALIFIED", ev: 0.08, label: "QUALIFIED" },
      })
    );
    assert.equal(d.executionActionable, false);
    assert.equal(d.evAvailable, false);
    assert.equal(d.qualification, "RESEARCH_ONLY");
  });

  it("does not crash on missing team identity", () => {
    const vm = buildBoardGameViewModel(
      researchNflGame({
        away: { abbr: "SEA" },
        home: null,
      })
    );
    assert.equal(vm.teams.identityOk, false);
    assert.equal(vm.projection.available, true);
  });
});

describe("PR #104 inherits corrected #103 market contract", () => {
  it("canonical default has no configured execution books", () => {
    assert.deepEqual([...DEFAULT_OPERATOR_EXECUTION_BOOKS], []);
  });

  it("no configured books → no execution (Heritage alone is not execution)", () => {
    const market = resolveCanonicalMarket({
      odds: {
        pinPresent: false,
        heritageListed: true,
        heritageHomeMl: -110,
        heritageAwayMl: -110,
        heritageSpreadHomePrice: -110,
        spread: 3,
        total: 47.5,
        heritageOverPrice: -110,
      },
    });
    assert.equal(market.executionMarketAvailable, false);
    assert.equal(market.execution.available, false);
    assert.equal(market.marketAvailable, true);
    assert.equal(market.authority.canEnterYourBet, false);

    const vm = buildBoardGameViewModel(
      researchNflGame({
        market: {
          marketAvailable: market.marketAvailable,
          execution: { available: false },
          consensus: {
            available: true,
            source: market.consensus?.source || "OBSERVED",
            observedOnly: true,
            spread: market.consensus?.spread ?? 3,
            total: market.consensus?.total ?? 47.5,
          },
          reference: { available: false },
          operatorExecutionBooks: [],
        },
        operatorExecutionBooks: [],
      })
    );
    assert.equal(vm.market.available, true);
    assert.ok(
      vm.market.label === "OBSERVED MARKET" || vm.market.label === "CONSENSUS"
    );
    assert.notEqual(vm.market.label, "BEST AVAILABLE");
    assert.notEqual(vm.market.label, "EXECUTION OFFER");
  });

  it("configured Heritage → execution works", () => {
    const market = resolveCanonicalMarket({
      odds: {
        pinPresent: false,
        heritageListed: true,
        heritageHomeMl: -110,
        heritageAwayMl: -110,
        heritageSpreadHomePrice: -110,
        spread: 3,
        total: 47.5,
        heritageOverPrice: -110,
      },
      operatorExecutionBooks: ["heritage"],
      marketObservedAt: new Date().toISOString(),
    });
    assert.equal(market.executionMarketAvailable, true);
    assert.equal(market.executionActionable, true);
    assert.equal(market.authority.canEnterYourBet, true);

    const vm = buildBoardGameViewModel(
      researchNflGame({
        market: {
          marketAvailable: true,
          execution: {
            available: true,
            book: market.execution.book || "Heritage",
            spread: market.execution.spread ?? 3,
            total: market.execution.total ?? 47.5,
            moneyline: market.execution.moneyline || { home: -110, away: -110 },
          },
          consensus: { available: false },
          reference: { available: false },
          operatorExecutionBooks: ["heritage"],
        },
        operatorExecutionBooks: ["heritage"],
      })
    );
    assert.equal(vm.market.label, "EXECUTION OFFER");
  });

  it("resolveBoardMarketLabel never invents BEST from role alone", () => {
    assert.equal(
      resolveBoardMarketLabel(
        researchNflGame({
          market: {
            marketAvailable: true,
            execution: { available: true, book: "FanDuel", spread: 3 },
            consensus: { available: false },
            reference: { available: false },
          },
        })
      ),
      "EXECUTION OFFER"
    );
  });
});
