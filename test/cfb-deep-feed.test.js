...
test("CFBD deep feed carries current form and matchup features into CFB-FBIS-v2", async () => {
  ...
  const fetchFn = async (url) => {
    ...
    if (path === "/stats/season/advanced") return response([
      { team:"Ohio State", offense:{ successRate:0.49, explosiveness:1.25, lineYards:3.4, pointsPerOpportunity:4.8, plays:72 }, defense:{ successRate:0.34, explosiveness:0.82, havoc:{total:0.19}, lineYards:2.4, stuffRate:0.22, pointsPerOpportunity:2.9 } },
      { team:"Michigan", offense:{ successRate:0.42, explosiveness:1.02, lineYards:3.0, pointsPerOpportunity:3.7, plays:66 }, defense:{ successRate:0.39, explosiveness:0.94, havoc:{total:0.15}, lineYards:2.8, stuffRate:0.18, pointsPerOpportunity:3.4 } },
    ]);
