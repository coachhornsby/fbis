import MlbValidationPanel from "./MlbValidationPanel.jsx";

export default function PopulationDescriptor({ descriptor, title = "Population descriptor" }) {
  const d = descriptor || {};
  const showMlbValidation =
    String(title || "").toLowerCase().includes("accuracy") &&
    (String(d.sport || "all").toLowerCase() === "mlb" || String(d.sport || "all").toLowerCase() === "all");

  return (
    <>
      {showMlbValidation ? <MlbValidationPanel /> : null}
