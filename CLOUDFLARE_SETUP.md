...
Heritage slip Confirm D1 write uses that same Pages secret: paste **HARVEST_SECRET** into the HARVEST_SECRET field in the import modal (`x-strategy-secret`). `STRATEGY_IMPORT_SECRET` is an optional override; if unset, `HARVEST_SECRET` is the write key. Never put the value in the client bundle.
