...
export function projectGame(sport, game) {
  ...
  const palMargin = game.bpp?.homeRuns != null && game.bpp?.awayRuns != null ? game.bpp.homeRuns - game.bpp.awayRuns : null;
  const palHome = game.bpp?.pHome != null ? game.bpp.pHome : palMargin != null ? logistic(palMargin, cfg.k) : null;
  const scoreHome = sport === "cfb" && game.cfb?.sigmaMargin != null && projMargin != null
    ? cfbWinProb(projMargin, game.cfb.sigmaMargin)
    : projMargin != null ? logistic(projMargin, cfg.k) : null;
  const homePct = recordWinPct(game.home?.record);
  const awayPct = recordWinPct(game.away?.record);
  const recordForm = homePct != null && awayPct != null && homePct + awayPct > 0 ? homePct / (homePct + awayPct) : null;
  const formHome = recordForm != null ? recordForm : game.modelHint?.formHome ?? null;
  const layers = { market: vigFree.home, espn: espnHome, score: scoreHome, pal: palHome, form: formHome };
  const model = {
    layers,
    ...
    pOver: sport === "cfb" && game.cfb?.sigmaTotal != null && game.odds?.total != null && projHome != null && projAway != null
      ? cfbTotalProb(projHome + projAway, game.odds.total, game.cfb.sigmaTotal, true) : null,
    pSpreadHome: sport === "cfb" && game.cfb?.sigmaMargin != null && game.odds?.spread != null && projMargin != null
      ? cfbSpreadProb(projMargin, game.odds.spread, game.cfb.sigmaMargin) : null,
...
export function projectionRecipe(sport, game, model) {
  ...
  if (sport === "mlb") {
    ...
    steps.push(`FBIS proprietary (Savant): ${game.away?.abbr || "Away"} ${fmt2(sv.awayRpg)} RPG vs ${homePitcher} → ${fmt(projAway)}; ${game.home?.abbr || "Home"} ${fmt2(sv.homeRpg)} RPG vs ${awayPitcher} × 1.04 → ${fmt(projHome)}.`);
    if (hasPal) {
      steps.push(`Ballpark Pal independent: ${game.away.abbr} ${fmt(game.bpp.awayRuns)} @ ${game.home.abbr} ${fmt(game.bpp.homeRuns)}.`);
      if (game.bpp.f5?.total != null) steps.push(`Pal F5 ${fmt(game.bpp.f5.awayRuns)}–${fmt(game.bpp.f5.homeRuns)} (tot ${fmt(game.bpp.f5.total)}).`);
    }
    ...
    steps.push(`Preseason prior ${c.priorVersion || "cfb-prior-v1"} (${c.projectionState || "n/a"}). Home rank ${c.homeEst?.rank ?? "unranked"} prior ${c.homeEst?.teamSpecificPrior ? "team-specific" : "missing"}; away ${c.awayEst?.rank ?? "unranked"} prior ${c.awayEst?.teamSpecificPrior ? "team-specific" : "missing"}.`);
    if (!c.bettingAllowed) steps.push(c.blockReason || CFB_BLOCKED_MESSAGE);
    steps.push(`Current-season weight n/(n+${c.constants?.priorGames ?? 6}): home ${c.homeEst?.n || 0} games w=${Number(c.homeEst?.w || 0).toFixed(2)}, away ${c.awayEst?.n || 0} games w=${Number(c.awayEst?.w || 0).toFixed(2)}.`);
    steps.push(`HFA ${c.hfa} pts${game.neutralSite ? " (neutral)" : ""}. Scores ${fmt(projAway)}–${fmt(projHome)} (tot ${fmt(c.total)}, mgn ${fmt(c.margin)}).`);
    steps.push(`Win/spread/total probabilities use Normal(σ_margin=${c.sigmaMargin}, σ_total=${c.sigmaTotal}) — labeled heuristic, not a fitted probability model.`);
    ...
    steps.push(`Feature stack (CFBD+ESPN): home [${homeFeatures.join(", ") || "none"}] · away [${awayFeatures.join(", ") || "none"}].`);
    if (homeQb?.starterKnown || awayQb?.starterKnown) steps.push(`QB continuity: ${game.home?.abbr || "HOME"} ${homeQb?.starterName || "unknown"}${homeQb?.starterTransfer ? " (transfer)" : ""}; ${game.away?.abbr || "AWAY"} ${awayQb?.starterName || "unknown"}${awayQb?.starterTransfer ? " (transfer)" : ""}.`);
    if (c.flags?.includes("feature_sparse")) steps.push("Some EPA/transfer/QB/coaching inputs were unavailable at runtime; absent features are omitted, not zero-filled.");
  } else if (sport === "nfl") {
    ...
    } else steps.push("FBIS projection unavailable. No independent NFL model is wired.");
  ...
  if (parts.length) steps.push(`Win-prob layers (${parts.join(" / ")}): market ${fmt2(layers.market)} · ESPN ${fmt2(layers.espn)} · score ${fmt2(layers.score)} · Pal ${fmt2(layers.pal)} · form ${fmt2(layers.form)}.`);
  return { engine, steps };
}
...
export function isQualifiedTicket(cfg, priced) {
  ...
  if (cfg.maxProb != null && (canonical.modelProbability > cfg.maxProb || canonical.modelProbability < 1 - cfg.maxProb)) return false;
...
function stampTicket(game, rec, priced, { qualified, lean }) {
  const hasExecutionPrice = validAmericanOdds(rec.executionPrice);
  const canonical = canonicalProbabilityFields(priced.fair);
  const qualificationBook = hasExecutionPrice ? EXECUTION_BOOK : "Pinnacle";
  const qualificationPrice = hasExecutionPrice ? Number(rec.executionPrice) : priced.pinPrice ?? null;
  return {
    ...rec,
    ...priced,
    qualified,
    ...
    book: hasExecutionPrice ? EXECUTION_BOOK : null,
    executionBook: hasExecutionPrice ? EXECUTION_BOOK : null,
    executionPrice: hasExecutionPrice ? Number(rec.executionPrice) : null,
    benchmarkBook: "Pinnacle",
    benchmarkPrice: priced.pinPrice ?? null,
    qualificationBook,
    qualificationPrice,
    qualificationSource: hasExecutionPrice ? "Heritage-execution" : "Pinnacle-qualified",
    priceSource: hasExecutionPrice ? "Heritage execution price" : "Pinnacle-qualified benchmark price",
...
export function recommendBundle(sport, game, model, weights) {
  const cfg = SPORTS[sport];
  if (sport === "cfb" && game.cfb && !game.cfb.bettingAllowed) return { qualified: null, lean: null, blocked: true, blockReason: game.cfb.blockReason || CFB_BLOCKED_MESSAGE };
  if (game.selectedChallenger && game.selectedChallenger.role === "shadow") return { qualified: null, lean: null, blocked: true, blockReason: SHADOW_BLOCK_REASONS.shadow };
  if (sport === "nfl" && game.projectionKind !== "FBIS") return { qualified: null, lean: null, blocked: true, blockReason: "FBIS projection unavailable — no independent NFL model" };
  if (game.marketUnresolved) return { qualified: null, lean: null, blocked: true, blockReason: TEAM_MATCH_UNRESOLVED };
  ...
  if (homeSpreadValid(game) && model.projMargin != null) {
    ...
    if (spreadEdge >= cfg.minSpreadEdge) {
      ...
      const priced = priceSelection({ pWin: pCover, twoWay: pin.spread, side: side === "HOME" ? "A" : "B", pinPrice: side === "HOME" ? game.odds.pinSpreadHomePrice : game.odds.pinSpreadAwayPrice });
      const base = { market: "SPREAD", side, pick: `${side === "HOME" ? (game.home.school || game.home.name) : (game.away.school || game.away.name)} ${fmtSpread(side === "HOME" ? homeSpread : -homeSpread)}`, line: side === "HOME" ? homeSpread : -homeSpread, executionPrice: heritageSpreadPrice(game, side), edge: priced.probEdge ?? spreadEdge };
  ...
  if (game.odds.total != null && model.projTotal != null) {
    ...
    if (Math.abs(diff) >= cfg.minSpreadEdge) {
      const over = diff > 0;
      const pRaw = sport === "cfb" && game.cfb?.sigmaTotal != null ? cfbTotalProb(model.projTotal, game.odds.total, game.cfb.sigmaTotal, over) : logistic(Math.abs(diff), cfg.totalK);
      const pMarket = over ? pin.total?.noVigA : pin.total?.noVigB;
      const p = shrinkToMarket(pRaw, pMarket);
      const priced = priceSelection({ pWin: p, twoWay: pin.total, side: over ? "A" : "B", pinPrice: over ? game.odds.pinOverPrice : game.odds.pinUnderPrice });
      const base = { market: "TOTAL", side: over ? "OVER" : "UNDER", pick: `${over ? "Over" : "Under"} ${game.odds.total}`, line: game.odds.total, executionPrice: heritageTotalPrice(game, over), edge: priced.probEdge ?? Math.abs(diff) };
      pushPriced(recs, cfg, game, base, priced, true);
    }
  }
  pushF5Recs(sport, game, recs, cfg, pin);
  if (sport === "cfb" && game.cfb?.projectionState === "PRIOR_ONLY") {
    const priorOnly = sortTickets(recs.map((r) => ({ ...r, qualified: false, lean: true, tag: "LEAN", reason: "PRIOR_ONLY CFB projection — value signal only; not eligible for a qualified or CONVICTION ticket" })));
  ...
  if (CONVICTION_QUALIFICATION_PAUSED) {
    const pausedLean = topLean || (topQualified ? { ...topQualified, qualified: false, lean: true, tag: "LEAN", pauseReason: CONVICTION_PAUSE_MESSAGE } : null);
    return { qualified: null, lean: pausedLean, blocked: false, qualificationPaused: true, pauseMessage: CONVICTION_PAUSE_MESSAGE, pausedCandidate: topQualified };
  }
...
function pushPriced(recs, cfg, game, base, priced, extraOk) {
  ...
  const hasExecutionPrice = validAmericanOdds(base.executionPrice);
  const qualificationPrice = hasExecutionPrice ? Number(base.executionPrice) : priced.pinPrice;
  const qualificationBook = hasExecutionPrice ? EXECUTION_BOOK : "Pinnacle";
  const qualificationEv = validAmericanOdds(qualificationPrice) ? expectedRoi(priced.fair, qualificationPrice) : null;
  const pricedForTicket = {
    ...priced,
    benchmarkPrice: priced.pinPrice ?? null,
    ev: qualificationEv,
    evPct: qualificationEv == null ? null : qualificationEv * 100,
    expectedRoiLabel: qualificationEv == null ? null : `Expected ROI ${qualificationEv >= 0 ? "+" : ""}${(qualificationEv * 100).toFixed(1)}% at ${qualificationBook}`,
    qualificationBook,
    qualificationPrice,
  };
  const qualified = validAmericanOdds(qualificationPrice) && isQualifiedTicket(cfg, pricedForTicket);
  if (qualified) {
  ...
  recs.push(stampTicket(game, { ...base, reason: base.reason || "Complete Pinnacle price required for qualification" }, pricedForTicket, { qualified: false, lean: true }));
}
...
function pushMl(recs, cfg, game, side, pick, priced) {
  ...
  pushPriced(recs, cfg, game, { market: "ML", side, pick, line: null, executionPrice: heritageMlPrice(game, side), implied: priced.implied ?? null, edge: edge ?? 0 }, priced, true);
}
...
function pushF5Recs(sport, game, recs, cfg, pin) {
  ...
  if (formHome != null && (f5Odds.homeMl != null || f5Odds.awayMl != null)) {
    ...
    if (homePriced.probEdge != null && homePriced.probEdge / 100 >= cfg.minMlEdge) pushPriced(recs, cfg, game, { market: "F5 ML", side: "HOME", pick: `${game.home.name} F5`, line: null, executionPrice: null, edge: homePriced.probEdge }, homePriced, true);
    if (formAway != null && awayPriced.probEdge != null && awayPriced.probEdge / 100 >= cfg.minMlEdge) pushPriced(recs, cfg, game, { market: "F5 ML", side: "AWAY", pick: `${game.away.name} F5`, line: null, executionPrice: null, edge: awayPriced.probEdge }, awayPriced, true);
  }
  ...
  if (palTotal != null && f5Odds.total != null) {
    ...
    if (Math.abs(diff) >= cfg.minSpreadEdge) {
      ...
      const priced = priceSelection({ pWin: shrinkToMarket(pF5Raw, pF5Mkt), twoWay: pin?.f5total, side: over ? "A" : "B", pinPrice: over ? f5Odds.overPrice : f5Odds.underPrice });
      const base = { market: "F5 TOTAL", side: over ? "OVER" : "UNDER", pick: `${over ? "Over" : "Under"} ${line} F5`, line, executionPrice: null, edge: priced.probEdge ?? Math.abs(diff) };
      pushPriced(recs, cfg, game, base, priced, true);
    }
  }
}

function homeSpreadValid(game) { return game.odds.spread != null; }
function fmtSpread(n) { if (n == null) return ""; return n > 0 ? `+${n}` : String(n); }
...
export function mapEvent(sport, event) {
  ...
  let projHome = null, projAway = null, marketProjHome = null, marketProjAway = null, projectionKind = "UNAVAILABLE";
  ...
  if (BASEBALL.has(sport)) projectionKind = "UNAVAILABLE";
  else if (sport === "nfl") projectionKind = marketProjHome != null ? "PINNACLE_IMPLIED" : "UNAVAILABLE";
  else if (sport === "cfb") projectionKind = "UNAVAILABLE";
  else if (marketProjHome != null) { projHome = marketProjHome; projAway = marketProjAway; projectionKind = "PINNACLE_IMPLIED"; }
  ...
  const mapped = { id: event.id, sport, start: event.date, status, home, away, odds, espnHomeWinPct, projHomeScore: projHome, projAwayScore: projAway, marketProjHome, marketProjAway, projectionKind, venue: comp.venue?.fullName || "", broadcast: (comp.broadcasts || []).map((b) => b.names?.[0] || b.market).filter(Boolean).join(", "), notes, week: num(event.week?.number) ?? num(event.season?.week) ?? null, neutralSite, conference: home.conference || away.conference || null };
...
export async function fetchEspnScoreboard(sport, date) {
  ...
  try {
    return await fetchJsonGuarded(url, { circuitKey: `espn-site:${sport}`, label: `ESPN ${cfg.label}`, retries: 1, timeoutMs: 9000, headers: { "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/128.0.0.0 Safari/537.36", Accept: "application/json,text/plain,*/*", "Accept-Language": "en-US,en;q=0.9", Referer: "https://www.espn.com/", Origin: "https://www.espn.com" }, shapeCheck: (json) => Array.isArray(json?.events) });
  } catch (err) { if (sport !== "cfb") throw err; }
  const fallback = `https://cdn.espn.com/core/college-football/scoreboard?xhr=1&dates=${stamp}&limit=300`;
  const json = await fetchJsonGuarded(fallback, { circuitKey: "espn-cfb-cdn", label: `ESPN ${cfg.label}`, retries: 1, timeoutMs: 9000, headers: { "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/128.0.0.0 Safari/537.36", Accept: "application/json,text/plain,*/*", Referer: "https://www.espn.com/" }, shapeCheck: (body) => Array.isArray(body?.events) || Array.isArray(body?.content?.sbData?.events) });
  return { ...(json || {}), events: json?.events || json?.content?.sbData?.events || [] };
}

const CIRCUIT = new Map();
function openCircuit(key, reason, ms = 120000) { CIRCUIT.set(key, { openedUntil: Date.now() + ms, reason }); }
function circuitBlocked(key) {
  const row = CIRCUIT.get(key);
  if (!row) return null;
  if (Date.now() > Number(row.openedUntil || 0)) { CIRCUIT.delete(key); return null; }
  return row;
}
async function fetchJsonGuarded(url, { circuitKey, label, retries = 0, timeoutMs = 9000, headers = {}, shapeCheck = null } = {}) {
  ...
  for (let attempt = 0; attempt <= retries; attempt += 1) {
    ...
    try {
      ...
      if (res.status === 403) { if (circuitKey) openCircuit(circuitKey, "403", 180000); throw new Error(`${label} 403`); }
      ...
      const txt = await res.text();
      let json = {};
      try { json = txt ? JSON.parse(txt) : {}; } catch { if (circuitKey) openCircuit(circuitKey, "malformed-json", 180000); throw new Error(`${label} malformed-json`); }
      if (shapeCheck && !shapeCheck(json)) throw new Error(`${label} invalid-json-shape`);
      return json;
    } catch (err) { lastErr = err; if (attempt < retries) await waitMs(300 * (attempt + 1)); }
...
function mapPitcher(p) {
  ...
  return { id: p.id || null, name, last: p.lastName || parts[parts.length - 1] || "", hand: p.pitchHand?.code || p.pitchHand?.description || "" };
}
function f5FromLinescore(g) {
  const innings = g.linescore?.innings || [];
  let home = 0, away = 0;
  const n = Math.min(5, innings.length);
  for (let i = 0; i < n; i++) { home += Number(innings[i].home?.runs) || 0; away += Number(innings[i].away?.runs) || 0; }
  return { home, away, innings: n, complete: n >= 5 };
}
function mapMlbStatsGame(g) {
  const home = g.teams?.home || {}, away = g.teams?.away || {}, ht = home.team || {}, at = away.team || {}, st = g.status || {};
  const abstract = st.abstractGameState || "", detailed = String(st.detailedState || ""), live = abstract === "Live";
  const completed = abstract === "Final" && !/postpone|cancel|suspend/i.test(detailed);
  const postponed = /postpone/i.test(detailed), canceled = /cancel/i.test(detailed), suspended = /suspend/i.test(detailed);
  const homeScore = num(home.score ?? g.linescore?.teams?.home?.runs), awayScore = num(away.score ?? g.linescore?.teams?.away?.runs);
  const homePct = Number(home.leagueRecord?.pct), awayPct = Number(away.leagueRecord?.pct);
  const formHome = Number.isFinite(homePct) && Number.isFinite(awayPct) && homePct + awayPct > 0 ? (homePct + 0.03) / (homePct + awayPct + 0.03) : 0.54;
  const mapped = {
    id: String(g.gamePk), sport: "mlb", start: g.gameDate,
    status: { state: live ? "in" : completed ? "post" : "pre", detail: st.detailedState || abstract || "", completed, live, postponed, canceled, suspended },
    home: { name: ht.name || "Home", abbr: ht.abbreviation || "H", logo: ht.id ? `https://www.mlbstatic.com/team-logos/${ht.id}.svg` : "", score: homeScore, rank: null, record: home.leagueRecord ? `${home.leagueRecord.wins}-${home.leagueRecord.losses}` : "", mlbId: ht.id || null },
    away: { name: at.name || "Away", abbr: at.abbreviation || "A", logo: at.id ? `https://www.mlbstatic.com/team-logos/${at.id}.svg` : "", score: awayScore, rank: null, record: away.leagueRecord ? `${away.leagueRecord.wins}-${away.leagueRecord.losses}` : "", mlbId: at.id || null },
    homeSp: mapPitcher(home.probablePitcher), awaySp: mapPitcher(away.probablePitcher), f5Score: f5FromLinescore(g),
    odds: { spread: null, total: null, homeMl: null, awayMl: null, details: "", book: EXECUTION_BOOK }, espnHomeWinPct: null, projHomeScore: null, projAwayScore: null,
    venue: g.venue?.name || "", broadcast: "", notes: [], modelHint: { formHome },
  };
...
function inferAbbr(name = "") {
  ...
  const picked = [parts[0], parts[parts.length - 1]].map((p) => p.replace(/[^A-Za-z]/g, "")).filter(Boolean).map((p) => p[0]);
...
async function fetchCfbdGamesSeason(year, apiKey) {
  if (!apiKey || !Number.isFinite(year) || year < 1869) return [];
  const url = new URL("https://api.collegefootballdata.com/games");
  url.searchParams.set("year", String(year)); url.searchParams.set("seasonType", "both"); url.searchParams.set("classification", "fbs");
  const res = await fetch(String(url), { headers: { Authorization: `Bearer ${apiKey}`, Accept: "application/json" } });
  if (!res.ok) throw new Error(`CFBD games ${res.status}`);
  const rows = await res.json();
  return Array.isArray(rows) ? rows : [];
}
async function fetchCfbdGamesRange(startDay, endDay, apiKey) {
  if (!apiKey || !startDay || !endDay) return [];
  const url = new URL("https://api.collegefootballdata.com/games");
  url.searchParams.set("startDate", startDay); url.searchParams.set("endDate", endDay); url.searchParams.set("seasonType", "both"); url.searchParams.set("classification", "fbs");
  const res = await fetch(String(url), { headers: { Authorization: `Bearer ${apiKey}`, Accept: "application/json" } });
  if (!res.ok) {
    if (res.status === 400) {
      const year = Number(String(startDay).slice(0, 4));
      const rows = await fetchCfbdGamesSeason(year, apiKey);
      const startMs = Date.parse(`${startDay}T00:00:00Z`), endMs = Date.parse(`${endDay}T23:59:59Z`);
      return rows.filter((row) => { const ymd = ymdCtForIso(row.startDate || row.start_date); const ms = Date.parse(`${ymd}T00:00:00Z`); return Number.isFinite(ms) && ms >= startMs && ms <= endMs; });
    }
...
async function fetchCfbdGamesForDate(day, apiKey) {
  ...
  return (Array.isArray(rows) ? rows : []).filter((g) => { const start = g.startDate || g.start_date; return ymdCtForIso(start) === day || String(start || "").slice(0, 10) === day; }).map((g) => mapCfbdGame(g));
}
export async function findNextCfbdGameDate(fromDay, apiKey, maxFutureDays = 14) {
  const futureDay = shiftDay(fromDay, maxFutureDays), rows = await fetchCfbdGamesRange(fromDay, futureDay, apiKey), todayMs = Date.parse(`${fromDay}T00:00:00Z`);
  if (!Number.isFinite(todayMs)) return null;
  let best = null;
  for (const row of rows) {
    const start = row.startDate || row.start_date, ymd = ymdCtForIso(start);
    if (!ymd) continue;
    const delta = Math.round((Date.parse(`${ymd}T00:00:00Z`) - todayMs) / 86400000);
    if (delta < 0 || delta > maxFutureDays) continue;
    if (!best || ymd < best) best = ymd;
  }
  return best;
}
export async function fetchCfbdGamesForWeek(anchorDay, apiKey, weekShift = 0) {
  const start = shiftDay(anchorDay, Number(weekShift || 0) * 7), end = shiftDay(start, 6), rows = await fetchCfbdGamesRange(start, end, apiKey), list = Array.isArray(rows) ? rows : [];
  ...
  const days = weekRows.map((g) => ymdCtForIso(g.startDate || g.start_date)).filter(Boolean).sort();
  return { week: selectedWeek, range: days.length ? { since: days[0], until: days[days.length - 1] } : null, games: weekRows.map((g) => mapCfbdGame(g)) };
}
...
function mapCfbdGame(g) {
  return attachMarketLabels(enrichGameTeams("cfb", {
    id: String(g.id), sport: "cfb", start: g.startDate || g.start_date || null,
    status: { state: g.completed ? "post" : "pre", detail: g.completed ? "Final" : "Scheduled", completed: Boolean(g.completed), live: false, postponed: false, canceled: false, suspended: false },
    home: { name: g.homeTeam || "Home", abbr: inferAbbr(g.homeTeam || ""), logo: "", score: num(g.homePoints), rank: null, record: "" },
    away: { name: g.awayTeam || "Away", abbr: inferAbbr(g.awayTeam || ""), logo: "", score: num(g.awayPoints), rank: null, record: "" },
    odds: { spread: null, total: null, homeMl: null, awayMl: null, details: "", book: EXECUTION_BOOK }, espnHomeWinPct: null, projHomeScore: null, projAwayScore: null, marketProjHome: null, marketProjAway: null, projectionKind: "UNAVAILABLE", venue: g.venue || "", broadcast: "", notes: [], week: num(g.week), neutralSite: Boolean(g.neutralSite), conference: null,
  }));
}
export function slimFinal(game) {
  return { id: String(game.id), sport: game.sport, start: game.start, home: { name: game.home?.name, abbr: game.home?.abbr, score: game.home?.score }, away: { name: game.away?.name, abbr: game.away?.abbr, score: game.away?.score }, status: game.status, f5Score: game.f5Score || null };
}
export async function fetchResults(sport, date) {
  const id = SPORTS[sport] ? sport : "mlb", day = date || todayCT();
  if (id === "mlb") { try { return (await fetchMlbStats(day)).map(slimFinal); } catch {} }
...
export async function fetchResultsForReconcile(sport, date, opts = {}) {
  try { return await fetchResults(sport, date); }
  catch (err) {
    if (sport === "cfb" && opts.cfbdApiKey) { try { return (await fetchCfbdGamesForDate(date || todayCT(), opts.cfbdApiKey)).map(slimFinal); } catch {} }
    throw err;
  }
}
export function dataQuality(sport, game) {
  ...
  if (sport === "mlb") {
    ...
    if (!game.bpp) flags.push("missing_pal"); else if (!game.bpp.lineupsOfficial) flags.push("lineups_unofficial");
    if (game.model?.layers?.pal != null && game.model?.layers?.score != null && Math.abs(game.model.layers.pal - game.model.layers.score) >= 0.08) flags.push("model_disagreement");
  ...
  if (sport === "cfb" && Array.isArray(game.cfb?.flags)) {
    ...
    if (game.cfb.projectionState === "LEAGUE_AVERAGE_ONLY") return { score: Math.min(12, Math.max(0, 100 - flags.length * 12)), flags };
  }
...
export async function buildSlate(sport, date, env = {}) {
  const id = SPORTS[sport] ? sport : "cbb", cfg = SPORTS[id], day = date || todayCT();
  let games = Array.isArray(env.prefetchedGames) ? env.prefetchedGames.slice() : [];
  if (id === "mlb") { try { games = await fetchMlbStats(day); } catch { games = []; } }
  if (!games.length) {
    try {
      const json = await fetchEspnScoreboard(id, day);
      games = (json.events || []).map((ev) => { const game = mapEvent(id, ev); return { ...game, model: projectGame(id, game) }; });
    } catch (err) {
      if (!games.length) {
        if (id === "cfb" && env.CFBD_API_KEY) games = await fetchCfbdGamesForDate(day, env.CFBD_API_KEY);
        if (!games.length && !env.PARLAY_API_KEY) throw err;
      }
    }
  }
  const parlay = await fetchParlayOdds(id, env.PARLAY_API_KEY, env.caches, { cacheOnly: Boolean(env.parlayCacheOnly), backupApiKey: env.THEODDS_API_KEY });
  games = mergeParlay(games, parlay.events, id);
  games = games.map((g) => attachMarketLabels(enrichGameTeams(id, g)));
  let pal = { games: [], meta: { enabled: false } }, savant = { meta: { enabled: false } }, cfb = { meta: { enabled: false } }, cbbd = { meta: { configured: false } };
  if (id === "mlb") {
    savant = await fetchSavantSlate(games, env.caches); games = savant.games || games;
    pal = await fetchBallparkPal(day, env.BALLPARK_PAL_API_KEY, env.caches, { cacheOnly: Boolean(env.palCacheOnly) }); games = mergeBallparkPal(games, pal);
  }
  if (id === "cfb") { cfb = await applyCfbModel(games, env); games = cfb.games || games; games = await attachCfbChallengers(games, env); }
  if (id === "cbb") {
    cbbd = await loadCbbdRatings(env);
    const attached = await attachCbbChallengers(games, env); games = attached.games;
  ...
  games = games.map((game) => {
    const pin = pinMarkets(game), model = projectGame(id, { ...game, pin }), next = { ...game, model, pin, modelVersion: MODEL_VERSION };
    return { ...next, quality: dataQuality(id, next) };
  });
  const live = games.filter((g) => g.status.live).length, final = games.filter((g) => g.status.completed).length, upcoming = games.length - live - final;
  return {
    sport: id, sportName: cfg.name, date: day, generatedAt: new Date().toISOString(), counts: { games: games.length, live, final, upcoming }, parlay: parlay.meta || { enabled: false }, pal: palSlateView(pal), savant: savant.meta || { enabled: false }, cfb: cfb.meta || { enabled: false }, cbbd: id === "cbb" ? cbbd.meta || { configured: false } : undefined, modelVersion: MODEL_VERSION, games,
    ticker: games.map((g) => ({ id: g.id, away: g.away.abbr, home: g.home.abbr, awayName: g.away.name, homeName: g.home.name, awayLogo: g.away.logo, homeLogo: g.home.logo, awayScore: g.away.score, homeScore: g.home.score, status: g.status.detail, live: g.status.live, completed: g.status.completed })),
  };
}
