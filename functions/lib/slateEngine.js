 ...
 * at the single recommendation boundary used by TODAY and the frozen ledger.
 */
...
const CRITICAL_QUALITY_FLAGS = new Set([
...
export function qualificationIntegrity(sport, game) {
  ...
  for (const flag of CRITICAL_QUALITY_FLAGS) {
    if (flags.has(flag)) {
      return {
        ok: false,
        reason: flag === "pinnacle_implied_score"
          ? "Qualification blocked — Pinnacle-implied scores are market context, not an independent model"
          : "Qualification blocked — market/team identity unresolved",
        code: flag,
      };
    }
  }
  ...
  if (id === "mlb") {
    for (const flag of MLB_STARTER_FLAGS) {
      if (flags.has(flag)) {
        return {
          ok: false,
          reason: "MLB qualification blocked — probable starter is unresolved",
          code: flag,
        };
      }
    }
...
export function recommendBundle(sport, game, model, weights) {
  ...
  if (candidate && !pinF5Complete(game, candidate.market)) {
    ...
    return {
      ...bundle,
      qualified: null,
      pausedCandidate: null,
      lean: bundle?.lean || leanFromCandidate(candidate, reason),
      blocked: false,
      integrityCode: "f5-pinnacle-two-way-required",
      blockReason: reason,
    };
  }
