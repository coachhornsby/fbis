 ...
 *
 * Public synthetic seed fixtures preserve cohort shape for offline tests.
...
export const STRATEGY_HC_V1 = {
  ...
  seedRevision: 1,
  seedDate: "2026-08-26",
  reportedRecord: "7-0", // public fixture graded record for synthetic seed; production uses D1
  ...
  seedObservation:
    "Public synthetic seed preserves the historical market mix (5 totals / 1 ML / 1 spread) as an observation for offline tests. Real operator positions are D1-only. Prospective tracking remains qualified CONVICTION / EV ≥ 8%.",
  notes:
...
/** Public synthetic 2026-08-26 CONVICTION fixtures (no real clubs / operator slips). */
...
export function strategyReconstruction(seedTickets = []) {
  ...
  // otherwise fall back to public synthetic fixtures for offline tests.
  const named =
  ...
  let note =
    "Operator-declared 2026-08-26 CONVICTION identity lives in Cloudflare D1. Public repo holds synthetic fixtures only. Journal EV, prices, and qualification timestamps are unrecovered until exact journal tickets are imported. Recovered N is not the reported 7-0.";
  ...
  if (seedN > EXPECTED_SEED_N || extras.length > 0 || invalidResultPattern(distinct)) {
    ...
    state = "unrecovered";
    note =
      "Identity is operator-declared (seven named 2026-08-26 CONVICTION picks in D1; synthetic fixtures for offline only). Journal fields unrecovered. Do not treat this as a reconstructed journal import.";
  }
